    ...
    public class DefaultFileProvider : AbstractVfsFileProvider
    {
        ...
        private LooseFileDiscovery IterateFiles(DirectoryInfo directory, SearchOption option)
        {
            ...
            foreach (var file in directory.EnumerateFiles("*.*", option))
            {
                ...
                {
                    if (file.FullName.Contains(@"Binaries\ThirdParty\CEF", StringComparison.OrdinalIgnoreCase) || file.FullName.Contains(@"Binaries\Win32\host") || file.FullName.Contains(@"Binaries\Win64\host") || file.FullName.Contains(@"NexonPlatformWebView\ThirdParty") || file.FullName.Contains(@"\qtwebengine_")) continue;
                    RegisterVfs(file);
                    continue;
                }
