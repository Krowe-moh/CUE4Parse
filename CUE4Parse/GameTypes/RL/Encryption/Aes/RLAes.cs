...
public static class RocketLeagueAes
{
    ...
    public static bool Decryptv2(byte[] inputData, int checkSumDataOffset, int checkSumDataSize, byte[] nonce, out byte[] outputData)
    {
        foreach (var key in KeyList)
        {
            if (!TryDecryptWithKey(inputData, key, checkSumDataOffset, checkSumDataSize, nonce, true, out var result))
                continue;
    ...
    public static void Decryptv2WithLastKey(byte[] inputData, byte[] nonce, out byte[] outputData)
    {
        if (!TryDecryptWithKey(inputData, _lastSuccessfulKey, 0, 0, nonce, false, out outputData))
            throw new InvalidAesKeyException("how?");
    }
}
