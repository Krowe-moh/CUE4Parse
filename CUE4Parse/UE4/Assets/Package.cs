...
namespace CUE4Parse.UE4.Assets
{
    ...
    public sealed class Package : AbstractUePackage
    {
        ...
        public FThumbnailTableItem[]? ThumbnailTable { get; }
            ...
            : base(uasset.Name.SubstringBeforeLast('.'), provider)
        {
            ...
            if (false && uassetAr.Game < GAME_UE4_0)
            {
                ...
                if (possibleTag == FPackageFileSummary.PACKAGE_FILE_TAG || possibleTag == 0x20000 || possibleTag == 0x10000)
                {
                    ...
                    var bWasByteSwapped = packageFileTag.CompressedSize == FPackageFileSummary.PACKAGE_FILE_TAG_SWAPPED ||
                                          packageFileTag.CompressedSize == (long) FArchive.BYTESWAP_ORDER64(FPackageFileSummary.PACKAGE_FILE_TAG);
            ...
            if (Summary is { ThumbnailTableOffset: > 0 })
            {
                ...
                ThumbnailTable = uassetAr.ReadArray(() => new FThumbnailTableItem(uassetAr));
            }
        ...
        {
            var span = ExportMap.AsSpan();
            for (var i = 0; i < span.Length; i++)
            {
                if (span[i].ObjectName.Text.Equals(name, comparisonType))
                    return i;
            }
        ...
        private ResolvedObject? ResolveImport(FPackageIndex importIndex)
        {
            ...
            var exportSpan = exportMap.AsSpan();
